...
{
    public function __construct(private readonly PlayStatsCalculator $playStatsCalculator) {}
    ...
    /**
     * The aggregation itself (rollup, ranking, breakdown...) lives in
     * PlayStatsCalculator - shared as-is with FriendPlayController::stats(),
     * which passes the friend User instead of the authenticated one. See
     * that class's own docblock for the "why" behind each field.
     */
    public function stats(Request $request): JsonResponse
    {
        return response()->json(['data' => $this->playStatsCalculator->calculate($request->user())]);
    }
}
